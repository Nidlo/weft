...
describe("<TermsReacceptDialog />", () => {
  it("renders nothing when the user's accepted version matches the live version", () => {
    useQuerySpy.mockReturnValue({
      data: { legalVersions: { termsVersion: "2026-04-01", privacyVersion: "2026-04-01" } },
    });
  ...
  it("renders the dialog when versions differ (stale acceptance)", () => {
    useQuerySpy.mockReturnValue({
      data: { legalVersions: { termsVersion: "2026-05-01", privacyVersion: "2026-04-01" } },
    });
  ...
  it("Accept button fires the mutation and refreshes the auth-store version", async () => {
    ...
    mutateSpy.mockResolvedValue({
      data: {
        acceptUpdatedTerms: { id: "user-1", termsAcceptedVersion: "2026-05-01" },
      },
    });

    render(<TermsReacceptDialog />);

    fireEvent.click(screen.getByRole("button", { name: /accept the updated terms/i }));
    ...
    await waitFor(() => {
      expect(useAuthStore.getState().user?.termsAcceptedVersion).toBe("2026-05-01");
    });
  });
