...
export function RatingBreakdown({ breakdown, averageRating, totalReviews }: RatingBreakdownProps) {
  const bars = [
      ...
      <div className="flex flex-1 flex-col gap-1.5">
        {bars.map(({ stars, count }) => (
          <div key={stars} className="flex items-center gap-2">
            <span className="w-4 text-right text-xs text-muted-foreground">{stars}</span>
